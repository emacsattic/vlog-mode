...
(defvar vlog-decl-type-words
  '("input" "output" "inout" "reg" "wire" "integer" "real"
    "signed" "unsigned" "event" "parameter" "defparam" )
...
;; (regexp-opt '("input" "output" "inout") t)
(defvar vlog-port-decl-re "\\<\\(\\(?:in[op]\\|outp\\)ut\\)\\>"
  "Regexp for port declaration.")

;; (regexp-opt '("reg" "wire" "signed" "unsigned") t)
(defvar vlog-port-decl-2-re "\\[[^]]+\\]\\|\\(reg\\|wire\\|signed\\|unsigned\\)\\>"
  "Another regexp for port declaration.")

(defgroup vlog-lib nil
  "Customizations for library routines."
  :group 'verilog)

(defcustom vlog-lib-sort-portlist-function 'identity
  "The function used to sort port list.
The port list is generated by `vlog-lib-sort-portlist-function'."
  :type  'function
  :group 'vlog-lib)

(defcustom vlog-lib-src-qualify-re "\\.v[lh]?\\'"
  "Regexp used to match for qualified verilog source buffers."
  :type  'string
  :group 'vlog-lib)
;;- constants & variables ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defun vlog-lib-grep (func lst)
  "Grep LST with FUNC, return the result list."
  (apply 'append (mapcar
...
(defun vlog-lib-buffer-qualified-p (buf)
  ...
  (and (bufferp buf)
       (string-match vlog-lib-src-qualify-re
...
(defun vlog-lib-get-module-ports-internal (beg end)
  ...
  (save-excursion
    ...
    (let (entries bound type)
      ...
      (while (vlog-re-search-forward vlog-port-decl-re end t)
        (setq bound (line-end-position)
              type  (match-string-no-properties 0))
        ;; Skip possible modifiers, such as `output reg unsigned out_port;'
        (vlog-skip-blank-and-useless-forward bound nil vlog-port-decl-2-re)
        ...
        (catch 'done
          (while (vlog-re-search-forward
            ...
            (setq entries
                  (append entries
                          (list (cons type (match-string-no-properties 3)))))
